...
export function BookingSuccess({ onBackToHome, onViewAccount }: BookingSuccessProps) {
  ...
  useEffect(() => {
    ...
    // If we have a bookingId from query params, fetch the booking data
    if (bookingId) {
      fetchBookingData(bookingId);
    } else if (!location.state?.bookingConfirmation) {
      // If no booking data in state or query params, redirect to home
      navigate('/');
    } else {
      // Use booking data from state
      setBooking(location.state.bookingConfirmation);
      setLoading(false);
    }
  }, [bookingId, message, navigate, location.state]);
